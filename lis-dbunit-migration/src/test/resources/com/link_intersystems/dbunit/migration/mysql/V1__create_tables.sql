...
create table actor_info (actor_id smallint default 0 not null, first_name varchar(45) not null, last_name varchar(45) not null, film_info text, constraint pk_actor_info primary key (actor_id, first_name, last_name));
...
create table film (film_id smallint not null, title varchar(255) not null, description text, release_year date, language_id smallint not null, original_language_id smallint, rental_duration smallint default 3 not null, rental_rate decimal(10,2) default 4.99 not null, length smallint, replacement_cost decimal(10,2) default 19.99 not null, rating varchar(255) default 'G', special_features varchar(255), last_update timestamp default CURRENT_TIMESTAMP not null, constraint pk_film primary key (film_id));
...
create table film_text (film_id smallint not null, title varchar(255) not null, description text, constraint pk_film_text primary key (film_id));
...
alter table actor_genre add constraint fk_actor_genre_actor foreign key (actor_id) references actor (actor_id);
...
alter table film add constraint fk_film_orig_language foreign key (original_language_id) references language (language_id);
...
alter table film_comment add constraint fk_film_comment_film foreign key (film_id) references film (film_id);
alter table film_genre add constraint fk_film_genre_film foreign key (film_id) references film (film_id);
alter table film_image add constraint fk_film_image_film foreign key (film_id) references film (film_id);
alter table film_origin add constraint fk_film_origin_film foreign key (film_id) references film (film_id);
alter table film_rating add constraint fk_film_rating_film foreign key (film_id) references film (film_id);
